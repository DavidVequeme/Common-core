...
/*   Updated: 2025/05/12 12:52:33 by dvidal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_format(int c, va_list argc, int *counter)
{
	if (c == '%')
		...
		ft_printb(va_arg(argc, unsigned int), "0123456789", 10, counter);
	else if (c == 'x')
		ft_printb(va_arg(argc, unsigned int), "0123456789abcdef", 16, counter);
	else if (c == 'X')
		ft_printb(va_arg(argc, unsigned int), "0123456789ABCDEF", 16, counter);
	else
		*counter = -1;
